...
def main():
    ...
    fps = ""
    target = [color_start, cir_start, Sequence_start]
    count = 0
    #response =cir_start##################################################################################################
    ...
    while True:
        ...
        if response == color_start:
            ...
            # 需要检测的圆环颜色列表
            colors_to_detect = ["red", "blue", "green", "general"]# 最后一个值为通用，按需求使用
            # 遍历颜色列表
            for color in colors_to_detect:
                F_text = Fdet.find_circle_Pos(frame, color)
                if F_text:
                    # 如果找到，退出循环
                    break
            run_text = "CIRCLE"
            ...
            # roi = img[y:y+h, x:x+w]
            F_text = Fdet.find_circle_Seq(frame)
            run_text = "Sequence"
            if F_text:
                res_text = remove(str(F_text))
                # response = None
